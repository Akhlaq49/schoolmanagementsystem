...
@Component({
  selector: 'app-teacher-attendance-edit',
  standalone: true,
...
export class TeacherAttendanceEditComponent implements OnInit {
  ...
  loadCorrections(): void {
    this.attendanceService.getMyTeacherCorrections().subscribe({
  ...
  submitCorrection(): void {
    if (!this.editingAttendance || !this.teacher || !this.requestReason.trim()) return;
    const teacherId = (this.teacher as any).userId ?? this.auth.getUserId();
    if (!teacherId) return;
    this.saving = true;
