...
@Component({
  ...
  template: `
    <div class="report-container">
      ...
      <div class="report-card" *ngIf="student">
...
export class AttendanceReportComponent implements OnInit {
  ...
  loadReport(): void {
    ...
    if (this.filterType === 'day') {
      ...
      this.attendanceService.getAttendanceReport(studentId, month, year).subscribe({
        next: (list) => {
    ...
    if (this.filterType === 'monthly' || this.filterType === 'yearly') {
      const month = this.filterType === 'yearly' ? 1 : this.selectedMonth;
      const year = this.selectedYear;
      if (this.filterType === 'yearly') {
        this.attendanceService.getAttendanceReport(studentId, 1, year).subscribe({
          next: (list) => {
            this.reportData = list.filter(a => {
              const y = typeof a.date === 'string' ? new Date(a.date).getFullYear() : (a.date as Date).getFullYear();
              return y === year;
            });
            this.reportPage = 1;
            this.loading = false;
